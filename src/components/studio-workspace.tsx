...
export function StudioWorkspace() {
  ...
  const selectedPreview = selectedScene?.shots.some((shot) => shot.id === selectedShotId)
    ? preview.clips.find((clip) => clip.shot_id === selectedShotId) ?? null
    : preview.clips.find((clip) => clip.scene_id === selectedScene?.id) ?? null;
  ...
  return <div className="studio-grid">
    <section className="brief-panel" aria-label="Описание фильма">
      ...
      <div className="estimate-lines"><div><span>{plan ? "Кадров по сценарию" : "Примерно кадров"}</span><strong>{plan ? scenes.reduce((sum, scene) => sum + scene.shots.length, 0) : estimate.shots}</strong></div><div><span>Генерация видео</span><strong>${estimate.videoUsd.toFixed(2)}</strong></div><div><span>Звук</span><strong>${estimate.audioUsd.toFixed(2)}</strong></div><div><span>Резерв на повторы</span><strong>${estimate.retriesReserveUsd.toFixed(2)}</strong></div><div className="budget-line"><span>Максимальный бюджет</span><label>$<input aria-label="Максимальный бюджет генерации" min="0" onChange={(event) => setBudget(Math.max(0, Number(event.target.value) || 0))} type="number" value={budget}/></label></div></div>
      ...
      <p className="approx-note">Одно подтверждение запускает весь цикл. Быстрый черновик автоматически переключает Veo 3.1 на Veo 3.1 Fast; Omni создаёт до 10 секунд за один запрос. Кадры публикуются сразу после ответа Google.{draft && duration <= 10 ? " Экспресс-цель для 10 секунд — до 3 минут; внешняя очередь Google может изменить фактическое время." : ""}</p>
    </section>
...
function TimelineTrack({ track, selectedShotId, onSelect }: { track: Track; selectedShotId: string; onSelect: (sceneId: string, shotId: string) => void }) { const Icon = track.icon; return <div className={`timeline-track tone-${track.tone}`}><div className="track-head"><Icon size={14}/><span>{track.label}</span><Eye size={13}/><Lock size={12}/></div><div className="track-clips">{track.clips.map((clip) => <button className={selectedShotId === clip.id.slice(2) ? "selected" : ""} key={clip.id} onClick={() => onSelect(clip.sceneId, clip.id.slice(2))} style={{ flexGrow: Math.max(.5,clip.duration) }} type="button"><span>{clip.label}</span></button>)}</div></div>; }
