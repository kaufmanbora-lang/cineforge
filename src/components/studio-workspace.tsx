  ...
  return <div className="studio-grid">
    <section className="brief-panel" aria-label="Описание фильма">
      ...
      <div className="control-stack">
        ...
        <div className="split-control"><label><span><ImageIcon size={15}/>Разрешение</span><select disabled={Boolean(plan)} onChange={(event) => setResolution(event.target.value as Resolution)} value={resolution}>{model.resolutions.map((entry) => <option key={entry} value={entry}>{entry === "preview" ? "Предпросмотр / черновик" : entry}</option>)}</select></label><label className="aspect"><span>Формат</span><select disabled={Boolean(plan)} onChange={(event) => setAspectRatio(event.target.value as AspectRatio)} value={aspectRatio}>{model.aspectRatios.map((entry) => <option key={entry}>{entry}</option>)}</select></label></div>
