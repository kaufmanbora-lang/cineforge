...
export function EditorWorkspace() {
  ...
  async function queueExport(sceneOnly = false) {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/${project.id}/export`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ format: "mp4", resolution, ...(sceneOnly ? { sceneId: selectedScene.id } : {}) }) });
