...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (projectId) {
      await query("UPDATE projects SET status='failed',last_error=$2,updated_at=now() WHERE id=$1", [projectId, JSON.stringify({ stage: "planning", message: error instanceof Error ? error.message : String(error) })]).catch(() => []);
    }
