...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (estimate.estimatedTotalUsd > maximumBudget) {
      return NextResponse.json({ error: "Estimated generation exceeds the project budget.", estimate, maximumBudget }, { status: 409 });
    }
    const plan = await latestMoviePlan(id);
    if (!plan) return NextResponse.json({ error: "Plan the movie before starting generation." }, { status: 409 });
