...
import { enqueueDialoguePatch, pauseProjectJobs, requeueDatabaseJob } from "@/server/movie/queue";
...
export async function processShot(databaseJobId: string): Promise<{ cached: boolean; storageKey: string; retrying?: boolean }> {
  const rows = await query<JobRow>(
    `SELECT j.*, p.model_id, p.resolution, p.aspect_ratio, p.maximum_budget_usd, p.spent_usd
  ...
  if (Number(job.spent_usd) >= Number(job.maximum_budget_usd)) {
  ...
  try {
    ...
    const cost = job.payload.shot.durationSeconds * price;
    ...
    if (qc && qc.overall < env().QC_RETRY_THRESHOLD && job.attempt + 1 < job.max_attempts) {
    ...
    const assetId = await persistCompletedAsset(job, storageKey, checksum, operation.output.bytes.byteLength, operation.operationId, cost, qc);
...
async function persistCompletedAsset(job: JobRow, storageKey: string, checksum: string, byteSize: number, operationId: string, cost: number, qc: ShotQcReport | null) {
  return transaction(async (client) => {
    ...
    await client.query("UPDATE jobs SET state='completed', completed_at=now(), result=$2 WHERE id=$1", [job.id, JSON.stringify({ storageKey, version })]);
    ...
    await client.query(
      "UPDATE projects SET completed_shots=$2, total_shots=$3, progress=$4, spent_usd=spent_usd+$5, status=CASE WHEN $2=$3 THEN 'validating'::project_status ELSE 'generating'::project_status END WHERE id=$1",
...
async function persistQcRetry(job: JobRow, input: {
  ...
  await transaction(async (client) => {
    ...
    await client.query(
      "UPDATE jobs SET state='retrying',payload=$2,last_error=$3,available_at=now()+($4::text || ' milliseconds')::interval WHERE id=$1",
    ...
    await client.query("UPDATE projects SET spent_usd=spent_usd+$2,status='generating' WHERE id=$1", [job.project_id, input.cost]);
    await client.query(
...
async function markCompleted(job: JobRow, storageKey: string, cost: number, cached: boolean) {
  await transaction(async (client) => {
    ...
    await client.query("UPDATE jobs SET state='completed',completed_at=now(),result=$2 WHERE id=$1", [job.id, JSON.stringify({ storageKey, cached })]);
    const count = await client.query<{ completed: number; total: number }>(
      "SELECT count(*) FILTER (WHERE state='completed')::int completed, count(*)::int total FROM shots WHERE project_id=$1",
      [job.project_id],
    );
    await client.query("UPDATE projects SET completed_shots=$2,total_shots=$3,progress=CASE WHEN $3=0 THEN 0 ELSE $2::numeric/$3*100 END,spent_usd=spent_usd+$4 WHERE id=$1", [
      job.project_id, count.rows[0].completed, count.rows[0].total, cost,
    ]);
  });
}
