...
import { MOVIE_QUEUE, pauseProjectJobs, recoverInterruptedJobs, redisConnection, requeueDatabaseJob } from "@/server/movie/queue";
...
async function handleBackgroundFailure(databaseJobId: string, type: string, error: unknown) {
  ...
  const job = rows[0];
  if (!job || job.state === "failed" || job.state === "paused" || job.state === "completed") return;
