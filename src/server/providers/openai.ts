...
Mark severePhysicalViolation=true only when directly visible: an impossible displacement or teleport; a person/object passing through or intersecting a wall, closed door, vehicle or other solid body; a persistent object appearing/disappearing; an unexplained location replacement at a continuous boundary; a doorway or gate becoming mirrored, moving to the other side, swapping hinge/handle edges, or changing swing direction without visible hinge rotation; or physically impossible anatomy/motion. A camera cut, lens change or reverse angle never changes world geometry. Do not call a motivated camera move or an explicitly planned hard cut a severe violation.
...
Write every character position as a topological blocking state, including inside/outside, side of wall or doorway, foreground/background, facing direction and intended travel path. A position may change only through a visible physically reachable path. For every recurring door or gate, locationState.objectPositions must carry one canonical topology record: wall plane, hinge edge as seen from the exterior, handle edge, inward/outward swing, opening angle, inside/outside sides and camera side of the action axis. Preserve that record across shots; opening changes only the angle around the same hinge. A camera cut must never mirror the doorway or swap its left/right edges. Explicitly identify who is on each side, who can reach which handle and in which direction the hinged door moves. Never script an impossible side change, passage through solid geometry or unexplained teleport. Use natural eyelines toward scene partners, props or travel direction; nobody looks into the camera unless direct address is explicitly requested.
